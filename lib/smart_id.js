...
exports.SmartIdSession = SmartIdSession
...
SmartId.prototype.certificate = function(id, opts) {
	return this.request("certificatechoice/" + identify(id), {
		...
		new SmartIdSession("cert", res.body.sessionID)
	))
}
...
SmartId.prototype.authenticate = function(id, signableHash, opts) {
	return this.request("authentication/" + identify(id), {
		...
		new SmartIdSession("auth", res.body.sessionID)
	))
}
...
SmartId.prototype.sign = function(id, signableHash, opts) {
	return this.request("signature/" + identify(id), {
		...
		new SmartIdSession("sign", res.body.sessionID)
	))
}
...
SmartId.prototype.wait = function(session, timeout) {
	...
	res = res.then(session.parse.bind(session))
	return res
}
...
function SmartIdSession(type, id) {
	this.type = type
	this.id = id
}

SmartIdSession.prototype.toJSON = function() {
	return {type: this.type, id: this.id}
}

SmartIdSession.prototype.parse = function(obj) {
	if (obj.state != "COMPLETE") return null

	var cert = new SmartIdCertificate(_.parseBase64(obj.cert.value))
	...
	switch (this.type) {
		...
		case "auth":
		case "sign": return [cert, _.parseBase64(obj.signature.value)]
		default: throw new RangeError("Invalid session type: " + this.type)
	}
}

SmartIdSession.parse = function(obj) {
	return new SmartIdSession(obj.type, obj.id)
}

SmartIdSession.serialize = function(session) { return session.toJSON() }
