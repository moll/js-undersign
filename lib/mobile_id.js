...
MobileId.prototype.certificate = function(phoneNumber, personalId) {
	...
	}).then(parse, handleFetchError.bind(null, null)).then((obj) => (
		new Certificate(_.parseBase64(obj.cert))
	))
}
...
MobileId.prototype.authenticate = function(
	...
	return this.request("authentication", {
		...
		handleFetchError.bind(null, null)
	)
}
...
MobileId.prototype.wait = function(session, timeout) {
	...
	return this.request(url, {timeout: timeoutMs + 5000}).then(
		parse,
		handleFetchError.bind(null, "SESSION_NOT_FOUND")
	).then(session.parse.bind(session))
}
...
MobileIdSession.prototype.parse = function(obj) {
	...
	var signature = _.parseBase64(obj.signature.value)

	switch (this.type) {
		case "auth": return [new Certificate(_.parseBase64(obj.cert)), signature]
...
function handleFetchError(notFoundCode, err) {
	if (err instanceof FetchError && err.response) {
		...
		switch (err.code) {
			case 400: throw new MobileIdError("BAD_REQUEST", msg, props)
			case 401: throw new MobileIdError("UNAUTHORIZED", msg, props)

			case 404:
				if (notFoundCode) throw new MobileIdError(notFoundCode, msg, props)
		}
	}
