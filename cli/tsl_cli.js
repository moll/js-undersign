...
Options:
    ...
    -f, --format=FMT     Format to print the OCSP response. [default: text]
  	--rfc4514            Use RFC 4514 names for certificates.
...
module.exports = co.wrap(function(argv) {
	...
	switch (fmt.toLowerCase()) {
		case "text": return void printTslText(tsl, {
			territory: args["<territory>"],
			rfc4514: args["--rfc4514"]
		})
	...
	providers.forEach(function(el, i) {
		...
		services.forEach(function(service, i) {
			...
			certs.forEach((cert) => console.log(indent(outdent`
				Certificate Name: ${opts.rfc4514
					? cert.subjectRfc4514Name
					: cert.subjectDistinguishedName.join(", ")
				}
			`)))
		})
	})
